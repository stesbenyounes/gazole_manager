...
def per_entry_consumption(entries):
    """
    Calcule L/100 km par plein: (litres du plein courant / km depuis plein précédent du même véhicule) * 100.
    Renvoie un dict {entry.id: valeur arrondie à 2 décimales}.
    """
...
def month_series_for(query):
    """
    Retourne (labels, liters, cost) groupés par YYYY-MM pour une query FuelEntry filtrée.
    """
    rows = (
        query.with_entities(
...
# Filtres mois (10 / octobre / 2025-10 / 10/2025)
MOIS_FR = {
...
def parse_month_to_range(s: str):
    ...
    s0 = s.strip().lower()
    try:
        if "-" in s0 and len(s0) == 7:         # "2025-10" (input type=month)
...
# -----------------------------------------------------------------------------
# Import des modèles APRÈS init_app pour éviter le circular import
# -----------------------------------------------------------------------------
with app.app_context():
    from models import Vehicle, Driver, FuelEntry, FuelType  # importe UNIQUEMENT les modèles
    db.create_all()
...
def entry_new():
    ...
    if request.method == 'POST':
        ...
        # retour vers la vue filtrée précédente
...
def entry_delete(eid):
    ...
    # préserver les filtres s’ils étaient présents
...
def vehicle_new():
    ...
    if request.method == 'POST':
        ...
        plate = plate or None  # éviter UNIQUE '' en doublon
        if name:
...
def driver_reports():
    ...
    # Calculer distances et km totaux
    distances = {}
    total_km = 0
    entries_by_vehicle = {}
...
def dashboard():
    ...
    # Filtre de période pour Top Chauffeurs
    ...
    # Top chauffeurs avec filtre de période
    ...
    for stat in driver_stats:
        # Calculer km parcourus et consommation moyenne pour ce chauffeur
        ...
        if total_km > 0:  # On garde seulement ceux avec des km
            top_drivers.append({
                'id': stat.id,
                'name': stat.name,
    ...
    # Trier par km parcourus (du plus au moins)
